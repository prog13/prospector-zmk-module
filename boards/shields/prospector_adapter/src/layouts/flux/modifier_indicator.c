...
#define MODIFIER_GRID_X (FLUX_LINES_GRID_OFFSET + MODIFIER_GRID_COL * FLUX_LINES_SPACING)
#define MODIFIER_CENTER_OFFSET_X (MODIFIER_GRID_X - FLUX_LINES_WIDTH / 2)

#define GRID_ROW_Y(row) (FLUX_LINES_GRID_OFFSET + (row) * FLUX_LINES_SPACING)
#define MODIFIER_CENTER_OFFSET_Y(row) (GRID_ROW_Y(row) - FLUX_LINES_HEIGHT / 2)
...
static void set_modifier_visible(lv_obj_t *obj, int grid_row, bool active, bool is_win_icon) {
    ...
    zmk_widget_flux_lines_set_cell_excluded(MODIFIER_GRID_COL, grid_row, true);
#else
    ...
    zmk_widget_flux_lines_set_cell_excluded(MODIFIER_GRID_COL, grid_row, active);
#endif
}
...
static lv_obj_t *create_modifier_container(lv_obj_t *parent, int grid_row) {
    lv_obj_t *container = lv_obj_create(parent);
    lv_obj_set_size(container, FLUX_LINES_SPACING, FLUX_LINES_SPACING);
...
#ifdef CONFIG_PROSPECTOR_SHOW_INACTIVE_MODIFIERS
    for (int i = 0; i < 4; i++) {
        zmk_widget_flux_lines_set_cell_excluded(MODIFIER_GRID_COL, grid_rows[i], true);
    }
#endif
